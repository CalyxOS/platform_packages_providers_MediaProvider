...
bool isUidAllowedToSeePathInternal(JNIEnv* env, jobject media_provider_object,
                                      jmethodID mid_is_uid_allowed_to_see_path_, uid_t uid,
                                      const string& path) {
    ScopedLocalRef<jstring> j_path(env, env->NewStringUTF(path.c_str()));
    bool res = env->CallBooleanMethod(media_provider_object, mid_is_uid_allowed_to_see_path_, uid,
                                      j_path.get());
...
std::vector<std::shared_ptr<DirectoryEntry>> getEntriesInDirectoryInternal(
...
MediaProviderWrapper::MediaProviderWrapper(JNIEnv* env, jobject media_provider) {
    ...
    mid_is_uid_allowed_to_see_path_ =
            CacheMethod(env, "isUidAllowedToSeePath", "(ILjava/lang/String;)Z");
...
std::vector<std::shared_ptr<DirectoryEntry>> MediaProviderWrapper::GetDirectoryEntries(
    ...
    JNIEnv* env = MaybeAttachCurrentThread();
    res = getEntriesInDirectoryInternal(env, media_provider_object_, mid_get_entries_in_dir_, uid, path);
    ...
    if (res_size && res[0]->d_name[0] == '/') {
        ...
        res.resize(0);
        addDirectoryEntriesFromLowerFs(dirp, /* filter */ nullptr, &res);
    } else if (!ENABLE_FUSE_DIRECTORY_PRIVACY && (res_size == 0 || !res[0]->d_name.empty())) {
...
bool MediaProviderWrapper::isUidAllowedToSeePath(uid_t uid, const string& path) {
    if (shouldBypassMediaProvider(uid)) {
        return true;
    }

    JNIEnv* env = MaybeAttachCurrentThread();
    return isUidAllowedToSeePathInternal(
            env, media_provider_object_, mid_is_uid_allowed_to_see_path_, uid, path);
}
