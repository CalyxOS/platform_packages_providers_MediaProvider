...
bool is_data_or_obb_path_with_default_ignorable_codepoints(const std::string_view& path) {
    if (__builtin_available(android 31, *)) {
        constexpr size_t primary_volume_prefix_len =
                std::size(mediaprovider::fuse::PRIMARY_VOLUME_PREFIX) - 1;
        ...
        if (path_beginning != mediaprovider::fuse::PRIMARY_VOLUME_PREFIX) {
            return false;
        }
        ...
        const std::string filtered_relevant_path =
                mediaprovider::fuse::removeDefaultIgnorableCodepoints(std::string(relevant_path));
        ...
        // codepoints), and if it's a private subpath (i.e. /0/Android/data).
