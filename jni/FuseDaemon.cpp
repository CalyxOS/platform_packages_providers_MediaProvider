...
struct fuse {
    explicit fuse(const std::string& _path, const ino_t _ino, const bool _uncached_mode,
        ...
        : path(_path),
          ...
          mp(0),
          zero_addr(0),
          disable_dentry_cache(mediaprovider::fuse::ENABLE_FUSE_PROBE_PROTECTION),
          passthrough(false),
...
static inline bool op_is_write(const FuseOp op) {
    switch (op) {
        case FuseOp::mknod:
        case FuseOp::mkdir:
        case FuseOp::create:
...
static std::unique_ptr<mediaprovider::fuse::FileLookupResult> validate_node_path(
    ...
    if (mediaprovider::fuse::ENABLE_FUSE_PROBE_PROTECTION
            && !fuse->mp->IsPathVisible(path, ctx->uid, op_is_write(op))) {
...
static node* do_lookup(fuse_req_t req, fuse_ino_t parent, const char* name,
    ...
    if (validate_access && !fuse->IsRoot(parent_node)) {
        const bool inaccessible_path = !is_app_accessible_path(fuse, parent_path, req->ctx.uid) ||
                (mediaprovider::fuse::ENABLE_FUSE_PROBE_PROTECTION &&
...
static void pf_access(fuse_req_t req, fuse_ino_t ino, int mask) {
    ...
    } else if (mediaprovider::fuse::ENABLE_FUSE_PROBE_PROTECTION) {
        std::unique_ptr<FileOpenResult> result = fuse->mp->OnFileOpen(
...
static void pf_create(fuse_req_t req,
    ...
    const string child_path = parent_path + "/" + name;
    if (mediaprovider::fuse::ENABLE_FUSE_PROBE_PROTECTION) {
        int status = fuse->mp->IsCreatingDirAllowed(child_path, req->ctx.uid);
        if (status) {
            // Caller might be trying to create a path that already exists. This would be caught
            // later as part of ensureFileColumns in MediaProvider.java, but in a way that reveals
            // the path's existence. Checking whether it's okay to create the path as a directory
            // suits our purposes here, for a file as well, and we can return with the same errors
            // that would be returned if the path did not exist, should such errors apply.
            fuse_reply_err(req, status);
            return;
        }
    }
