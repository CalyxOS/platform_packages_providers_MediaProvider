...
static constexpr size_t PRIMARY_VOLUME_PREFIX_LEN = std::size(PRIMARY_VOLUME_PREFIX) - 1;
...
bool is_data_or_obb_path_with_ignorable_codepoints(const string& path, struct fuse* fuse) {
    ...
    if (__builtin_available(android 31, *)) {
        ...
        for (UChar32 c = utext_next32From(ut, PRIMARY_VOLUME_PREFIX_LEN);
            ...
            if (textPhase >= -1
                    && u_hasBinaryProperty(c, UProperty::UCHAR_DEFAULT_IGNORABLE_CODE_POINT)) {
                // From the user ID section to the end of our checking, ignorable codepoints
                // are relevant.
                hasAnyIgnorable = true;
                continue;
            }
            if (textPhase == -1) {
                // This is the user ID section. We don't care what it is.
                if (c == '/') {
                    textPhase++;
                }
                continue;
        ...
        if (textPhase == std::size(textPhaseData) - 1) {
            // Matched to the end, with no trailing slash (optional final phase).
            utext_close(ut);
            return hasAnyIgnorable;
        }
        utext_close(ut);
