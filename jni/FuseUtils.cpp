...
std::string removeDefaultIgnorableCodepoints(const std::string& str) {
    // These libicu unicode methods require SDK 31 or above. Otherwise, we return the same string.
    if (__builtin_available(android 31, *)) {
        // Temporarily do nothing
        if ((true)) return std::string(str);
